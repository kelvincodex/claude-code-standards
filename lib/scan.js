 ...
 */
function getProjectExtensions(stack, projectDir) {
  const exts = [];
...
function scanTestingPatterns(projectDir, stack) {
  ...
  const testFiles = collectFiles(projectDir, testExtensions, 50);
...
/**
 * Read actual rules from existing lint/format config files.
 * Returns an object like { Prettier: ['single quotes', '2-space indent', 'trailing commas'], ... }
 */
function scanConfigDetails(projectDir) {
  const details = {};

  // Prettier
  const prettierFiles = ['.prettierrc', '.prettierrc.json', 'prettier.config.js', '.prettierrc.js'];
  for (const file of prettierFiles) {
    const content = readSafe(path.join(projectDir, file));
    if (!content) continue;
    try {
      // Handle JSON configs
      const cfg = JSON.parse(content);
      const rules = [];
      if (cfg.singleQuote !== undefined) rules.push(cfg.singleQuote ? 'single quotes' : 'double quotes');
      if (cfg.semi !== undefined) rules.push(cfg.semi ? 'semicolons' : 'no semicolons');
      if (cfg.tabWidth) rules.push(`${cfg.tabWidth}-space indent`);
      if (cfg.useTabs) rules.push('tabs');
      if (cfg.trailingComma) rules.push(`trailing commas: ${cfg.trailingComma}`);
      if (cfg.printWidth) rules.push(`print width: ${cfg.printWidth}`);
      if (rules.length > 0) details['Prettier'] = rules;
    } catch {}
    break;
  }

  // Biome
  const biomeContent = readSafe(path.join(projectDir, 'biome.json')) || readSafe(path.join(projectDir, 'biome.jsonc'));
  if (biomeContent) {
    try {
      const cfg = JSON.parse(biomeContent);
      const rules = [];
      const fmt = cfg.formatter || {};
      if (fmt.indentStyle) rules.push(`indent: ${fmt.indentStyle}`);
      if (fmt.indentWidth) rules.push(`${fmt.indentWidth}-space indent`);
      if (fmt.lineWidth) rules.push(`line width: ${fmt.lineWidth}`);
      const js = cfg.javascript?.formatter || {};
      if (js.quoteStyle) rules.push(`${js.quoteStyle} quotes`);
      if (js.semicolons) rules.push(js.semicolons === 'always' ? 'semicolons' : 'no semicolons');
      if (js.trailingCommas) rules.push(`trailing commas: ${js.trailingCommas}`);
      if (rules.length > 0) details['Biome'] = rules;
    } catch {}
  }

  // EditorConfig
  const editorconfig = readSafe(path.join(projectDir, '.editorconfig'));
  if (editorconfig) {
    const rules = [];
    const indentMatch = editorconfig.match(/indent_style\s*=\s*(\w+)/);
    const indentSizeMatch = editorconfig.match(/indent_size\s*=\s*(\w+)/);
    const eolMatch = editorconfig.match(/end_of_line\s*=\s*(\w+)/);
    if (indentMatch) rules.push(`indent: ${indentMatch[1]}`);
    if (indentSizeMatch) rules.push(`indent size: ${indentSizeMatch[1]}`);
    if (eolMatch) rules.push(`EOL: ${eolMatch[1]}`);
    if (rules.length > 0) details['EditorConfig'] = rules;
  }
...
/**
 * Scan monorepo workspaces — detect and summarize each workspace's stack.
 */
function scanWorkspaces(projectDir) {
  const workspaces = [];
  const pkgPath = path.join(projectDir, 'package.json');
  ...
  if (workspacePatterns.length === 0) {
    const pnpmWs = readSafe(path.join(projectDir, 'pnpm-workspace.yaml'));
    if (pnpmWs) {
      const matches = pnpmWs.match(/- ['"]?([^'"]+)['"]?/g);
      if (matches) {
        workspacePatterns = matches.map(m => m.replace(/^- ['"]?/, '').replace(/['"]?$/, ''));
      }
    }
  }
  ...
  for (const pattern of workspacePatterns) {
    ...
    if (!fs.existsSync(fullBaseDir) || !fs.statSync(fullBaseDir).isDirectory()) continue;
    ...
    if (pattern.endsWith('*')) {
      try {
        ...
        for (const entry of entries) {
          if (entry.isDirectory() && !entry.name.startsWith('.')) {
            const wsDir = path.join(fullBaseDir, entry.name);
            const wsPkg = readSafe(path.join(wsDir, 'package.json'));
            if (wsPkg) {
              try {
                const pkg = JSON.parse(wsPkg);
                const deps = Object.keys({ ...pkg.dependencies, ...pkg.devDependencies });
                const frameworks = [];
                if (deps.includes('next')) frameworks.push('Next.js');
                else if (deps.includes('react')) frameworks.push('React');
                ...
                if (deps.includes('tailwindcss')) frameworks.push('Tailwind');
                workspaces.push({
      ...
      // Direct path (e.g., "packages/shared")
      const wsPkg = readSafe(path.join(fullBaseDir, 'package.json'));
      if (wsPkg) {
        try {
          const pkg = JSON.parse(wsPkg);
          workspaces.push({
            name: pkg.name || baseDir,
            path: baseDir,
            frameworks: [],
          });
        } catch {}
...
/**
 * Load and run custom plugin scanners from .claude-standards/scanners/
 */
function runPluginScanners(projectDir, files) {
  const pluginDir = path.join(projectDir, '.claude-standards', 'scanners');
  if (!fs.existsSync(pluginDir)) return {};

  const pluginResults = {};
  try {
    const entries = fs.readdirSync(pluginDir).filter(f => f.endsWith('.js'));
    for (const entry of entries) {
      try {
        ...
        if (typeof plugin.scan === 'function') {
          ...
          pluginResults[name] = plugin.scan(projectDir, files, { readSafe, collectFiles, sampleFiles });
        }
      } catch (err) {
        pluginResults[`_error_${entry}`] = `Plugin error: ${err.message}`;
      }
    }
  } catch {}
...
/**
 * Scan for console.log/warn/error debris.
 */
function scanConsoleLogs(projectDir, files) {
  let total = 0;
  const fileResults = [];
 ...
 */
function scanTodoComments(projectDir, files) {
  let total = 0;
  const fileResults = [];
  ...
  for (const file of files) {
    const content = readSafe(file);
    if (!content) continue;
    const matches = content.match(/\/\/\s*(TODO|FIXME|HACK|XXX)\b|\/\*[\s\S]*?(TODO|FIXME|HACK|XXX)/g);
    if (matches && matches.length > 0) {
      total += matches.length;
      fileResults.push({
        path: path.relative(projectDir, file),
...
/**
 * Measure import alias compliance (e.g., @/ vs ../).
 */
function scanImportAliasCompliance(projectDir, files) {
  let aliasImports = 0;
  ...
  for (const file of files) {
    ...
    const aliasMatches = content.match(/from\s+['"]@\//g);
    if (aliasMatches) aliasImports += aliasMatches.length;

    const relMatches = content.match(/from\s+['"]\.\.\//g);
    if (relMatches) relativeImports += relMatches.length;
  }

  const totalImports = aliasImports + relativeImports;
  const ratio = totalImports > 0 ? Math.round((aliasImports / totalImports) * 100) : 0;

  let compliance = 'N/A';
  if (totalImports > 0) {
    if (ratio >= 90) compliance = 'STRICT';
    else if (ratio >= 70) compliance = 'GOOD';
    else if (ratio >= 50) compliance = 'PARTIAL';
    else compliance = 'LOW';
  }

  return { aliasImports, relativeImports, ratio, compliance };
}
...
function scanAll(projectDir, stack) {
  const exts = getProjectExtensions(stack, projectDir);
  ...
  const results = {
    ...
    configDetails: scanConfigDetails(projectDir),
    ci: scanCI(projectDir),
    monorepo: scanMonorepo(projectDir),
    workspaces: scanMonorepo(projectDir).isMonorepo ? scanWorkspaces(projectDir) : [],
    plugins: runPluginScanners(projectDir, allFiles),
    consoleLogs: scanConsoleLogs(projectDir, allFiles),
    todoComments: scanTodoComments(projectDir, allFiles),
    importAliasCompliance: scanImportAliasCompliance(projectDir, allFiles),
  };
...
function formatScanResults(results) {
  ...
  // Config details (actual rules from Prettier/Biome/EditorConfig)
  if (results.configDetails && Object.keys(results.configDetails).length > 0) {
    lines.push('### Tool Config Rules');
    lines.push('These are the actual rules from your config files — AI tools must respect them:');
    for (const [tool, rules] of Object.entries(results.configDetails)) {
      lines.push(`- **${tool}**: ${rules.join(', ')}`);
    }
    lines.push('');
  }
  ...
  if (results.monorepo.isMonorepo) {
    ...
    if (results.workspaces && results.workspaces.length > 0) {
      ...
      for (const ws of results.workspaces) {
        const fwk = ws.frameworks.length > 0 ? ` (${ws.frameworks.join(', ')})` : '';
        lines.push(`  - \`${ws.path}\` — ${ws.name}${fwk}`);
      }
    }
    lines.push('');
  }
  ...
  // Plugin results
  if (results.plugins && Object.keys(results.plugins).length > 0) {
    const validPlugins = Object.entries(results.plugins).filter(([k]) => !k.startsWith('_error_'));
    if (validPlugins.length > 0) {
      lines.push('### Custom Patterns');
      for (const [name, data] of validPlugins) {
        if (typeof data === 'string') {
          lines.push(`- **${name}**: ${data}`);
        } else if (Array.isArray(data)) {
          lines.push(`- **${name}**: ${data.join(', ')}`);
  ...
  if (results.importAliasCompliance) {
    const iac = results.importAliasCompliance;
    if (iac.aliasImports > 0 || iac.relativeImports > 0) {
      lines.push('### Import Alias Compliance');
      lines.push(`- Alias imports (@/): ${iac.aliasImports} (${iac.ratio}%)`);
      lines.push(`- Relative imports (../): ${iac.relativeImports}`);
      lines.push(`- Compliance: ${iac.compliance}`);
      lines.push('');
    }
  }

  // Code Hygiene
  const hasHygiene = (results.consoleLogs && results.consoleLogs.total > 0) ||
                     (results.todoComments && results.todoComments.total > 0);
  if (hasHygiene) {
    lines.push('### Code Hygiene');
    if (results.consoleLogs && results.consoleLogs.total > 0) {
      const cl = results.consoleLogs;
      const worst = cl.files[0];
      lines.push(`- Console statements: ${cl.total} across ${cl.files.length} file(s)${worst ? ` (worst: ${worst.path} — ${worst.count})` : ''}`);
    }
    if (results.todoComments && results.todoComments.total > 0) {
      const td = results.todoComments;
      lines.push(`- TODO/FIXME comments: ${td.total} across ${td.files.length} file(s)`);
    }
    lines.push('');
  }
...
/**
 * Format code hygiene data as standalone section for template injection.
 */
function formatCodeHygiene(results) {
  const lines = [];

  if (results.consoleLogs && results.consoleLogs.total > 0) {
    const cl = results.consoleLogs;
    lines.push(`- Console statements: ${cl.total} across ${cl.files.length} file(s)`);
    for (const f of cl.files.slice(0, 5)) {
      lines.push(`  - \`${f.path}\`: ${f.count}`);
    }
  } else {
    lines.push('- Console statements: none detected');
  }

  if (results.todoComments && results.todoComments.total > 0) {
    const td = results.todoComments;
    lines.push(`- TODO/FIXME: ${td.total} across ${td.files.length} file(s)`);
  } else {
    lines.push('- TODO/FIXME: none detected');
  }

  if (results.importAliasCompliance) {
    const iac = results.importAliasCompliance;
    if (iac.aliasImports > 0 || iac.relativeImports > 0) {
      lines.push(`- Import alias compliance: ${iac.ratio}% (${iac.compliance})`);
    }
  }
