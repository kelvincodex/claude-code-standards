...
/**
 * Generate CLAUDE.md from template with stack-aware sections and scanned patterns.
 */
function generateClaudeMd(projectDir, stack, scanResults) {
  ...
  const output = processTemplate(template, stack, scanResults);

  const destPath = path.join(projectDir, 'CLAUDE.md');

  // On update: preserve manual sections
  if (fs.existsSync(destPath)) {
    const existing = fs.readFileSync(destPath, 'utf-8');

    // If there's an auto-discovered section, only update that
    ...
    if (existing.includes(autoMarker) && scanResults) {
      ...
      console.log('  Updated Learned Patterns section (manual edits preserved)');
      return;
    }

    // Otherwise backup and overwrite
...
/**
 * Generate AGENTS.md (universal format for Cursor, Copilot, Codex, etc.)
 */
function generateAgentsMd(projectDir, stack, scanResults) {
  const templatePath = path.join(TEMPLATES_DIR, 'AGENTS.md.template');
...
/**
 * Generate .cursorrules (Cursor AI format)
 */
function generateCursorRules(projectDir, stack, scanResults) {
  const templatePath = path.join(TEMPLATES_DIR, 'cursorrules.template');
  if (!fs.existsSync(templatePath)) return;

  const template = fs.readFileSync(templatePath, 'utf-8');
  const output = processTemplate(template, stack, scanResults);

  const destPath = path.join(projectDir, '.cursorrules');
  if (fs.existsSync(destPath)) {
    fs.copyFileSync(destPath, destPath + '.backup');
  }
  fs.writeFileSync(destPath, output);
