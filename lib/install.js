...
const { formatScanResults, formatCodeHygiene } = require('./scan');
...
function processTemplate(template, stack, scanResults) {
  ...
  if (scanResults) {
    ...
    output = output.replace('{{CODE_HYGIENE}}', formatCodeHygiene(scanResults));
  } else {
    output = output.replace('{{LEARNED_PATTERNS}}', '');
    output = output.replace('{{CODE_HYGIENE}}', '');
  }
 ...
 * Checks for custom template overrides in .claude-standards/templates/ first.
 */
function generateFromTemplate(templateName, stack, scanResults, projectDir) {
  // Check for custom override first
  if (projectDir) {
    const overridePath = path.join(projectDir, '.claude-standards', 'templates', templateName);
    if (fs.existsSync(overridePath)) {
      const template = fs.readFileSync(overridePath, 'utf-8');
 ...
 * Returns an array of diff lines with +/- prefixes, or empty array if identical.
 */
...
function resolveFormats(format) {
  ...
  // Accept an array directly (from interactive prompt)
  if (Array.isArray(format)) {
    return format.filter(f => allKeys.includes(f));
  }
  ...
  // Unknown format — warn and fall back
  console.warn(`Warning: Unknown format "${format}". Valid formats: ${['all', 'popular', ...allKeys].join(', ')}`);
 ...
 */
function generateAllFormats(projectDir, stack, scanResults, format, updateOnly = false, dryRun = false) {
  ...
  for (const key of formatKeys) {
    ...
    if (FULL_FORMATS[key]) {
      ...
      content = generateFromTemplate(fmt.template, stack, scanResults, projectDir);
      ...
      destPath = path.join(projectDir, fmt.path);
      const body = generateFromTemplate(fmt.template, stack, scanResults, projectDir);
      if (body) content = fmt.frontmatter + body;
    }

    if (!content) continue;

    // --update mode: skip files that don't already exist
    if (updateOnly && !fs.existsSync(destPath)) {
      continue;
    }

    const fmtPath = FULL_FORMATS[key]?.path || CONDENSED_FORMATS[key]?.path || FRONTMATTER_FORMATS[key]?.path;

    // Dry-run mode: report what would be written, skip actual writes
    if (dryRun) {
      const action = fs.existsSync(destPath) ? 'update' : 'create';
      console.log(`  [dry-run] Would ${action} ${fmtPath}`);
      generated.push(key);
      continue;
    }
    ...
    if (key === 'claude' && fs.existsSync(destPath)) {
      ...
      if (existing.includes(autoMarker) && scanResults) {
        ...
        // Show diff preview
        const diff = simpleDiff(existing, updated);
        if (diff.length === 0) {
          console.log('  CLAUDE.md — no changes detected');
          generated.push(key);
          continue;
        }
        console.log('  CLAUDE.md — Learned Patterns changes:');
        for (const line of diff.slice(0, 30)) {
          if (line.startsWith('+')) console.log(`    \x1B[32m${line}\x1B[0m`);
          else if (line.startsWith('-')) console.log(`    \x1B[31m${line}\x1B[0m`);
        }
        if (diff.length > 30) console.log(`    ... and ${diff.length - 30} more lines`);
    ...
    // Add version stamp (only on initial creation, not CLAUDE.md update path)
    const stamp = `<!-- Generated by claude-code-standards v${version} -->\n\n`;
    content = stamp + content;

    writeWithBackup(destPath, content);
    generated.push(key);
    console.log(`  Created ${fmtPath}`);
  }
